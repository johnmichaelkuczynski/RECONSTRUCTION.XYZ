...
- **Core Services**:
    ...
    - **Coherence Meter**: Offers simple chunking and outline-guided processing with specialized modes:
      - **Mathematical Coherence Type**: Performs DUAL analysis - (1) standard coherence analysis checking logical structure and consistency, and (2) veridicality analysis checking if the theorem being proved is actually TRUE. Displays both scores with subscores for claim truth, inference validity, boundary conditions, and overall soundness. Shows counterexamples and mathematical flaws when detected.
      - **Mathematical Proof Rewrite**: Corrects proofs using Claude with extended thinking. If the theorem is TRUE, fixes the proof. If FALSE, finds a similar true theorem and proves that instead. Returns theorem status, corrected proof, and key corrections.
