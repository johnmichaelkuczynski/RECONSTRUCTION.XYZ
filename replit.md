...
- **Axiom Library**: NEW - Browsable library of pre-built axiom sets (e.g., KVA-0: Rational Choice Foundations). Users can click "Axiom Library" button to load axiom sets directly into the Text Model Validator input field. Library data stored in `client/src/lib/axiomLibrary.ts`.
- **ZHI Model Names**: Model labels now display as "ZHI 1", "ZHI 2", etc. without parenthetical backend names (GPT-4, Claude, etc.) for cleaner branding.
